            ...
            <div class="col-md-12">
                <div class="card border-0">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h1 class="text-center mx-auto mb-0">Active Users List</h1>
                    </div>
                                        ...
                                        <tr>
                                            ...
                                            <td class="align-middle">
                                                @if ($user->status == 'pending')
                                                    <span class="badge bg-primary status-modal cursor-pointer"
                                                        data-bs-toggle="modal" data-bs-target="#statusModal"
                                                        data-user-id="{{ $user->id }}">{{ $user->status }}</span>
                                                @elseif ($user->status == 'active')
                                                    <span class="badge bg-success text-dark status-modal p-2" style="cursor: pointer"
                                                        data-bs-toggle="modal" data-bs-target="#statusModal"
                                                        data-user-id="{{ $user->id }}">{{ $user->status }}</span>
                                                @elseif ($user->status == 'blocked')
                                                    <span class="badge bg-danger status-modal cursor-pointer"
                                                        data-bs-toggle="modal" data-bs-target="#statusModal"
                                                        data-user-id="{{ $user->id }}">{{ $user->status }}</span>
                                                @else
